...
Solver::Solver(PowerSystem &system, const Options &opts)
...
Solver::Solver(PowerSystem &system)
	: system_(system)
{
	options_ = Options();
	size_t n = system_.nodesCount();
...
std::vector<double> Solver::calculateMismatches(const Matrix<std::complex<double>> &Y_bus) const
	{
...
double Solver::calc_P_calc(const Matrix<std::complex<double>> &Y_bus, const size_t i) const
{
	double calced = 0.;
...
double Solver::calc_Q_calc(const Matrix<std::complex<double>> &Y_bus, const size_t i) const
{
	double calced = 0.;
	for (size_t j = 0; j < system_.nodesCount(); ++j) {
		double V = V_mag_[i] * V_mag_[j];
...
Matrix<double> Solver::buildJacobian(const Matrix<std::complex<double>> &Y_bus) const
{
...
void Solver::updateVoltages(const std::vector<double> &dx)
{
...
std::vector<double> Solver::solveLinearSystem(Matrix<double> J, std::vector<double> F) const
{
...
bool Solver::checkPVLimits(const Matrix<std::complex<double>> &Y_bus){
